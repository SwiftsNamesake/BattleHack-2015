...
--        - Factor out common pieces of logic such as finding (origin, size) of a particular key
...
keybounds :: PianoSettings -> Int -> (Complex Double, Complex Double)
keybounds piano i = case keyLayout i of
  KeyAccidental -> (piano-->keysize.real', (2*piano-->indent):+(piano-->mid))
  _             -> (0:+0, piano-->keysize)
